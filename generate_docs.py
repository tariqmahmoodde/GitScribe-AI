...
SYSTEM_PROMPT = (
    ...
    "- NEVER mention AI, Groq, any AI model, or that this README was "
    "auto-generated. Write as if a human developer authored it.\n"
    "- Do NOT include any footer, badge, or note referencing automation tools.\n"
)
...
def generate_readme(context: str, model: str) -> str:
    ...
    readme = _strip_attribution(_strip_thinking(raw))
...
# Keywords that betray AI/tool attribution — any line containing these is dropped.
_ATTRIBUTION_PATTERNS = re.compile(
    r"(generated by|auto.?generated|groq|qwen|gitscribe ai|powered by|"
    r"using groq|ai model|language model|llm)",
    re.IGNORECASE,
)


def _strip_attribution(text: str) -> str:
    """Drop any line that references AI generation or tooling."""
    lines = text.splitlines()
    clean = [line for line in lines if not _ATTRIBUTION_PATTERNS.search(line)]
    return "\n".join(clean).strip()
